...
/*   Updated: 2025/06/01 16:58:59 by bsalim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_dead(t_philo *philo)
{
    ...
    if(get_current_time() - philo->data->time_to_start > philo->data->time_to_die)
    {
...
{
    if(philo->data->most_meals_should_philo_eat > 0 && philo->meals_eaten == philo->data->number_of_philosophers)
    {
        pthread_mutex_unlock(&philo->data->meals_mutex);
        pthread_mutex_destroy(&philo->data->protect_stop_sumilation);
        return (-1);
    }
    return (1);
}
